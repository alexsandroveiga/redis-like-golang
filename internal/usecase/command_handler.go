...
func NewCommandHandler(parser *protocol.Parser, store repository.KeyValueRepository, persist repository.PersistenceRepository, stats *Stats) *CommandHandler {
...
func (h *CommandHandler) ExecuteCommand(ctx context.Context, cmd *protocol.Command) string {
	h.stats.IncrementCommands() // ERRO
...
func (h *CommandHandler) handleGet(ctx context.Context, args []string) string {
	if len(args) != 1 {
		return h.parser.FormatError("GET requires at least 1 argument")
	}
...
func (h *CommandHandler) handleExists(ctx context.Context, args []string) string {
	if len(args) < 1 {
		return h.parser.FormatError("EXIST requires at least 1 argument")
	}
	count := 0
	for _, key := range args {
...
func (h *CommandHandler) handleExpire(ctx context.Context, args []string) string {
	if len(args) < 1 {
		return h.parser.FormatError("EXPIRE requires at least 1 argument")
	}
	key := args[0]
...
func (h *CommandHandler) handlePersist(ctx context.Context, args []string) string {
	if len(args) < 1 {
		return h.parser.FormatError("PERSIST requires at least 1 argument")
	}
...
func (h *CommandHandler) handleTTL(ctx context.Context, args []string) string {
	if len(args) != 1 {
		return h.parser.FormatError("TTL requires at least 1 argument")
	}
